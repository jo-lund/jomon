...
void print_netstat()
{
    ...
    read_netstat();
    calculate_rate();
    printat(stat_screen->win, y, 0, hdrcol | A_BOLD, "Network statistics for %s", ctx.device);
    mvwprintw(stat_screen->win, ++y, 0, "");
    printat(stat_screen->win, ++y, 0, subcol | A_BOLD, "%13s", "Upload rate");
    wprintw(stat_screen->win, ": %8.2f kB/s", tx.kbps);
    wprintw(stat_screen->win, "\t%4d packets/s", tx.pps);
    printat(stat_screen->win, ++y, 0, subcol | A_BOLD, "%13s", "Download rate");
    ...
    if (get_iw_stats(ctx.device, &iwstat) && get_iw_range(ctx.device, &iwrange)) {
        mvwprintw(stat_screen->win, ++y, 0, "");
        printat(stat_screen->win, ++y, 0, subcol | A_BOLD, "%13s", "Link quality");
        wprintw(stat_screen->win, ": %8u/%u", iwstat.qual.qual, iwrange.max_qual.qual);
        printat(stat_screen->win, ++y, 0, subcol | A_BOLD, "%13s", "Level");
        wprintw(stat_screen->win, ": %8d dBm", (int8_t) iwstat.qual.level);
        printat(stat_screen->win, ++y, 0, subcol | A_BOLD, "%13s", "Noise");
        wprintw(stat_screen->win, ": %8d dBm", (int8_t) iwstat.qual.noise);
    }
    if (show_packet_stats) {
        mvwprintw(stat_screen->win, ++y, 0, "");
        if (pstat[0].num_packets) {
            printat(stat_screen->win, ++y, 0, subcol | A_BOLD, "%23s %12s", "Packets", "Bytes");
            for (int i = 0; i <= NUM_PROTOCOLS; i++) {
                if (pstat[i].num_packets) {
                    printat(stat_screen->win, ++y, 0, subcol | A_BOLD, "%13s", pstat[i].protocol);
...
void print_hwstat()
{
    ...
    read_hwstat();
    printat(stat_screen->win, y, 0, hdrcol, "Memory and CPU statistics");
    mvwprintw(stat_screen->win, ++y, 0, "");
    printat(stat_screen->win, ++y, 0, subcol, "%20s", "Total memory");
    wprintw(stat_screen->win, ": %8lu kB", hw.total_ram);
    printat(stat_screen->win, ++y, 0, subcol, "%20s", "Memory used");
    wprintw(stat_screen->win, ": %8lu kB", hw.total_ram - hw.free_ram);
    printat(stat_screen->win, -1, -1, subcol, "%10s", "Buffers");
    wprintw(stat_screen->win, ": %6lu kB", hw.buffers);
    printat(stat_screen->win, -1, -1, subcol, "%8s", "Cache");
    wprintw(stat_screen->win, ": %8lu kB", hw.cached);
    mvwprintw(stat_screen->win, ++y, 0, "");
    printat(stat_screen->win, ++y, 0, subcol, "%20s", "Process memory (RSS)");
    wprintw(stat_screen->win, ": %8lu kB", hw.vm_rss);
    printat(stat_screen->win, -1, -1, subcol, "%6s", "Pid");
    wprintw(stat_screen->win, ": %d", hw.pid);
    mvwprintw(stat_screen->win, ++y, 0, "");
    if (cpustat[0][0].idle != 0 && cpustat[0][1].idle != 0) {
        for (int i = 0; i < hw.num_cpu; i++) {
            idle = cpustat[i][!cpuidx].idle - cpustat[i][cpuidx].idle;
            printat(stat_screen->win, ++y, 0, subcol, "CPU%d idle", i);
