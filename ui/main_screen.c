...
void show_selectionbar(main_screen *ms, WINDOW *win, int line, uint32_t attr)
{
    ...
    mvwchgat(win, line, 0, -1, attr, get_theme_colour(SELECTIONBAR), NULL);
}
...
void main_screen_get_input(screen *s)
{
    ...
    case KEY_END:
        if (interactive) {
            ...
            show_selectionbar(ms, ms->base.win, ms->selection_line - ms->top, A_NORMAL);
            wrefresh(ms->base.win);
        }
        break;
    case KEY_F(1):
...
void print_header(main_screen *ms)
{
    ...
    if (ctx.filename[0]) {
        printat(ms->header, y, 0, COLOR_PAIR(txtcol) | A_BOLD, "Filename");
        wprintw(ms->header, ": %s", ctx.filename);
    } else {
        printat(ms->header, y, 0, COLOR_PAIR(txtcol) | A_BOLD, "Device");
        wprintw(ms->header, ": %s", ctx.device);
    }
    inet_ntop(AF_INET, &local_addr->sin_addr, addr, sizeof(addr));
    printat(ms->header, ++y, 0, COLOR_PAIR(txtcol) | A_BOLD, "Local address");
...
void print_status(main_screen *ms)
{
    ...
    printat(ms->status, -1, -1, COLOR_PAIR(colour), "%-11s", "Help");
    wprintw(ms->status, "F2");
    printat(ms->status, -1, -1, COLOR_PAIR(colour), "%-11s", "Menu");
    if (ctx.capturing || euid != 0) {
        printat(ms->status, -1, -1, COLOR_PAIR(disabled) | A_BOLD, "F3");
    } else {
        wprintw(ms->status, "F3");
    }
    printat(ms->status, -1, -1, COLOR_PAIR(colour), "%-11s", "Start");
    if (ctx.capturing) {
        wprintw(ms->status, "F4");
    } else {
        printat(ms->status, -1, -1, COLOR_PAIR(disabled) | A_BOLD, "F4");
    }
    printat(ms->status, -1, -1, COLOR_PAIR(colour), "%-11s", "Stop");
    if (ctx.capturing || vector_size(packets) == 0) {
        printat(ms->status, -1, -1, COLOR_PAIR(disabled) | A_BOLD, "F5");
        printat(ms->status, -1, -1, COLOR_PAIR(colour), "%-11s", "Save");
    } else {
        wprintw(ms->status, "F5");
        printat(ms->status, -1, -1, COLOR_PAIR(colour), "%-11s", "Save");
    }
    if (ctx.capturing) {
        printat(ms->status, -1, -1, COLOR_PAIR(disabled) | A_BOLD, "F6");
        printat(ms->status, -1, -1, COLOR_PAIR(colour), "%-11s", "Load");
    } else {
    ...
    if (view_mode == DECODED_VIEW) {
        printat(ms->status, -1, -1, COLOR_PAIR(colour), "%-11s", "View (dec)");
    } else {
        printat(ms->status, -1, -1, COLOR_PAIR(colour), "%-11s", "View (hex)");
    }
    wprintw(ms->status, "F10");
    printat(ms->status, -1, -1, COLOR_PAIR(colour), "%-11s", "Quit");
    wrefresh(ms->status);
}
...
void goto_line(main_screen *ms, int c)
{
    ...
    if (isdigit(c) && num < INT_MAX / 10) {
        ...
        if (num >= ms->top && num < ms->top + my) {
            remove_selectionbar(ms, ms->base.win, ms->selection_line, A_NORMAL);
            if (ms->subwindow.win && num > ms->top + ms->subwindow.top) {
                ms->selection_line = num - 1 + ms->subwindow.num_lines;
            } else {
                ms->selection_line = num - 1;
            }
            show_selectionbar(ms, ms->base.win, ms->selection_line, A_NORMAL);
        } else {
            werase(ms->base.win);
            remove_selectionbar(ms, ms->base.win, ms->selection_line, A_NORMAL);
