...
{
    ldiv_t val;
    long int hours, mins, secs, msecs;

    val = ldiv(ms, 60 * 60 * 1000);
    hours = val.quot;
    val = ldiv(val.rem, 60 * 1000);
    mins = val.quot;
    val = ldiv(val.rem, 1000);
    secs = val.quot;
    msecs = val.rem;
    snprintf(buf, n, "%02ld:%02ld:%02ld.%03ld", hours, mins, secs, msecs);
    return buf;
}
...
struct tm_t get_time(uint32_t num_secs)
{
    ...
    val = ldiv(num_secs, 60 * 60 * 24);
    t.days = val.quot;
    val = ldiv(val.rem, 60 * 60);
    t.hours = val.quot;
    val = ldiv(val.rem, 60);
    t.mins = val.quot;
    t.secs = val.rem;
...
void time_ntop(struct tm_t *time, char *result, size_t len)
{
    ...
    memset(result, 0, len);
    if (time->days) {
        n += snprintcat(result, len, time->days == 1 ? "%ld day, " : "%ld days, ",
                       time->days);
    }
    if (time->hours) {
        n += snprintcat(result, len, time->hours == 1 ? "%ld hour, " : "%ld hours, ",
                        time->hours);
    }
    if (time->mins) {
        n += snprintcat(result, len, time->mins == 1 ? "%ld minute, " :
                        "%ld minutes, ", time->mins);
    }
    if (time->secs) {
        n += snprintcat(result, len, time->secs == 1 ? "%ld second" :
                        "%ld seconds", time->secs);
    }
    if (!n) {
